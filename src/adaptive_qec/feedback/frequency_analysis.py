...
class FrequencyAnalyzer:
    ...
    def __init__(
        self,
        Ki: float = 0.05,
        latency_s: float = 500e-9,  # 500 ns
        plant_gain: float = 1.0,
        plant_pole_hz: float = 1000.0  # 1 kHz pole
    ):
        self.Ki = Ki
    ...
    def closed_loop_tf(self, freq_hz: np.ndarray) -> np.ndarray:
        """
        Compute closed-loop transfer function T(jω).
    ...
    def compute_margins(
        ...
        if crossover_idx > 0 and crossover_idx < len(freq) - 1:
        ...
        if phase_cross_idx > 0 and phase_cross_idx < len(freq) - 1:
        ...
        # Stable if both margins positive

...
class CosmicRaySimulator:
    ...
    def __init__(
        self,
        distance: int = 7,
        impact_rate: float = 0.5,  # 0.5 per 1000 cycles
    ...
    def generate_impact(
        ...
        -------
        CosmicRayImpact
            The generated impact event.
        """
        if center is None:
            center = np.random.randint(0, self.num_qubits)
        
        if radius is None:
            # Vary radius around typical
    ...
    def _get_qubits_in_radius(self, center: int, radius: int) -> List[int]:
        """Get all qubits within radius of center."""
    ...
    def get_depolarization_map(
        ...
        Depolarization decreases with distance from impact center.
    ...
    def apply_to_noise_model(
        ...
        """
        Get modified noise rates after applying impact.
        ...
        impact : CosmicRayImpact
            The impact to apply.
        base_depol : float
            Base depolarization rate.
            
        Returns
        -------
        np.ndarray
            Per-qubit depolarization rates.
        """
        depol_map = self.get_depolarization_map(impact)
        return base_depol + depol_map
...
def inject_cosmic_ray(
    ...
    This is a convenience function for testing burst detection.
    
    Parameters
    ----------
    syndromes : np.ndarray
        Original syndrome data.
    center : int
        Center qubit of impact.
    radius : int
        Impact radius.
    flip_probability : float
        Probability of flipping each affected detector.
        
    Returns
    -------
    np.ndarray
        Modified syndromes with burst injected.
    """
    modified = syndromes.copy()
    
    # Flip detectors near center
